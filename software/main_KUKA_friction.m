%% KUKA LWR 4+ Robot - Simulation with both uncertainty on the masses and unmodeled frictions!
...
mod_value = 30;
lin_value = 2;              % set when upgrading the linearization point (iterative) 
...
% nominal robot
% kuka_nominal = kuka;                                                  % perfect model 
...
% desired Cartesian trajectories: 
...
damping = 0; % this corresponds to the classical pseudo-inverse solution
% damping = 0.01;
...
%   ---------------------------------------------------------------------------------------
    ...
    if(not(isempty(Xdata)) && (gp_flag == 1) && length(gp_1.KernelInformation.KernelParameters) == 22 )    
        ...
        % subtract the GP prediction to the reference acceleration (the - sign is according to how the dataset labels have been collected)
    ...
    % compute friction
    friction(i,:) = (alpha_f*(1./(1+exp(-30*dq(i,:))))' - 1/2*ones(7,1) + beta_f*dq(i,:)')';
    
    % dynamic model inversion - row vector
    ddq(i,:) = inv(massMatrix(kuka,q(i,:))) * (TauFL - velocityProduct(kuka,q(i,:),dq(i,:)) - gravityTorque(kuka,q(i,:)) - friction(i,:) )';
    ...
    DeltaNi =  velocityProduct(kuka,q(i,:),dq(i,:)) + gravityTorque(kuka,q(i,:)) + friction(i,:) - velocityProduct(kuka_nominal,q(i,:),dq(i,:)) - gravityTorque(kuka_nominal,q(i,:)); 
    ...
    needed_torques_masses = vertcat(needed_torques_masses, Torque_needed_masses);
...
if save_data == 1
    ...
    if gp_flag == 1
        data_path = ['results/' model '/' traj '/linGP/opt/friction/' ];
        ...
        dlmwrite(fullfile(data_path, 'delta_eta.txt'), delta_eta)
        dlmwrite(fullfile(data_path, 'needed_torques_masses.txt'), needed_torques_masses)
        dlmwrite(fullfile(data_path, 'friction.txt'), friction)
        disp('Data successfully saved!')
    elseif gp_flag == 0
        data_path = ['results/' model '/' traj '/no_learning/friction/' ];
        dlmwrite(fullfile(data_path, 'q.txt'), q)
        dlmwrite(fullfile(data_path, 'dq.txt'), dq)
        dlmwrite(fullfile(data_path, 'ddq.txt'), ddq)
        dlmwrite(fullfile(data_path, 'p.txt'), task_vec(:,1:3))
...
if save_workspace == 1
    ...
    if gp_flag == 1
        if opt_flag == 1
            save([workspace_path 'linGP_friction'])
        elseif opt_flag == 0
            save([workspace_path 'linGP_noOpt_friction'])
        end
    elseif gp_flag == 0
        save([workspace_path 'no_learning_friction'])
    end
...
%% friction plot
if gp_flag == 1
    t2 = t(lin_value+1:end-1);

    f4 = figure();
    f4.WindowState = 'maximized';
    subplot(4,1,1)
    plot(t2, corrective_torques(lin_value+1:end,1) - needed_torques_masses(lin_value+1:end,1), 'b', 'Linewidth', 1.5), hold on
    plot(t2, delta_eta(lin_value+1:end,1), 'r', 'Linewidth', 1.5)
    ...
    subplot(4,1,2)
    plot(t2, corrective_torques(lin_value+1:end,2) - needed_torques_masses(lin_value+1:end,2), 'b', 'Linewidth', 1.5), hold on
    plot(t2, delta_eta(lin_value+1:end,2), 'r', 'Linewidth', 1.5)
    title('2^{nd} joint')
    xlabel('time [s]'),
    ylabel('torque  [Nm]'),
    grid on
    ...
    subplot(4,1,3)
    plot(t2, corrective_torques(lin_value+1:end,3) - needed_torques_masses(lin_value+1:end,3), 'b', 'Linewidth', 1.5), hold on
    plot(t2, delta_eta(lin_value+1:end,3), 'r', 'Linewidth', 1.5)
    title('3^{rd} joint')
    xlabel('time [s]'),
    ylabel('torque  [Nm]'),
    grid on
    ...
    subplot(4,1,4)
    plot(t2,corrective_torques(lin_value+1:end,4) - needed_torques_masses(lin_value+1:end,4), 'b', 'Linewidth', 1.5), hold on
    plot(t2, delta_eta(lin_value+1:end,4), 'r', 'Linewidth', 1.5)
    title('4^{th} joint')
    xlabel('time [s]'),
    ylabel('torque  [Nm]'),
    grid on
    ...
    subplot(3,1,1)
    plot(t2, corrective_torques(lin_value+1:end,5) - needed_torques_masses(lin_value+1:end,5), 'b', 'Linewidth', 1.5), hold on
    plot(t2, delta_eta(lin_value+1:end,5), 'r', 'Linewidth', 1.5)
    title('5^{th} joint')
    xlabel('time [s]'),
    ylabel('torque  [Nm]'),
    legend('estimated', 'real', 'Location', 'best', 'FontSize', 12, 'Orientation', 'horizontal');
    ...
    subplot(3,1,2)
    plot(t2, corrective_torques(lin_value+1:end,6) - needed_torques_masses(lin_value+1:end,6), 'b', 'Linewidth', 1.5), hold on
    plot(t2, delta_eta(lin_value+1:end,6), 'r', 'Linewidth', 1.5)
    title('6^{th} joint')
    xlabel('time [s]'),
    ylabel('torque  [Nm]'),
    grid on
    ...
    subplot(3,1,3)
    plot(t2, corrective_torques(lin_value+1:end,7) - needed_torques_masses(lin_value+1:end,7) , 'b', 'Linewidth', 1.5), hold on
    plot(t2, delta_eta(lin_value+1:end,7), 'r', 'Linewidth', 1.5)
    title('7^{th} joint')
    xlabel('time [s]'),
    ylabel('torque  [Nm]'),
    grid on
