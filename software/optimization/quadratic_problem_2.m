%% LQ variance optimization by splitting time t from time t+1 (FINAL APPROACH)

function [H, f] = quadratic_problem_2(q_input, dq_input, DeltaT, x_lin, Xdata, Ydata, sigma1, sigma2, sigma3, sigma4, sigma5, sigma6, sigma7, beta, sigmaN)
    ...
    bt = [1; (q_input - x_lin(1:7))'; (dq_input - x_lin(8:14))'; (-x_lin(15:21))'];
    At = [zeros(1,7); zeros(7,7); zeros(7,7); eye(7,7)];
    Ht = 2*At'*V*At;
    ft = (2*bt'*V*At)';
    
    % minimizing variance at time t+1
    b_next = [1; (q_input - x_lin(1:7) + dq_input*DeltaT)'; (dq_input - x_lin(8:14))'; (-x_lin(15:21))'];
    A_next = [zeros(1,N); eye(N)*DeltaT^2 ; eye(N)*DeltaT; zeros(N)];   
    H_next = A_next' * (2*V) * A_next;
    f_next = (2*b_next'*V*A_next)';
        
    % recall:   - quadratic problem are w.r.t ddq_t, not x_hat_t and x_hat_t+1
